...
`static $OJ_BBS=false;`           |  论坛的形式，`"discuss3"` 为自带的简单论坛，`"bbs"` 为外挂论坛，`false` 为关闭。
`static $OJ_ONLINE=false;`            |  是否使用在线监控，需要消耗一定的内存和计算，因此如果并发大建议关闭。
`static $OJ_LANG="cn";`              |  默认的语言，中文为 `cn` 。
`static $OJ_SIM=false;`               |  是否显示相似度检测的结果（显示只是界面开关，检测实际开关在 judge.conf）。
`static $OJ_DICT=true;`               |  是否启用在线英字典。
`static $OJ_LANGMASK=33554356;`           |  用掩码表示OJ接受的提交语言，可以被比赛设定覆盖。掩码计算器：https://pigeon-developer.github.io/hustoj-langmask/
...
`static $OJ_CSS="white.css";`           |  默认css，可选：`white.css` `bing.css` `kawai.css` `black.css` `blue.css` `green.css` `hznu.css` 等。
`static $OJ_SAE=false;`               |  是否是在新浪的云平台运行web 部分
`static $OJ_VCODE=false;`             |  是否启用图形登录、注册验证码。
`static $OJ_APPENDCODE=true;`        |  是否启用自动添加代码，启用的话，提交时会参考$OJ_DATA 对应目录里是否有 `append.c` 一类的文件，有的话会把其中代码附加到对应语言的答案之后，巧妙使用可以指定 `main` 函数而要求学生编写main 部分调用的函数。
