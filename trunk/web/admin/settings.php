...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // P0: CSRF token validation. Matches the HUSTOJ standard pattern used by
    // other admin pages (problem_import_md, problem_edit, ...). The token is
    // generated by set_post_key.php inside the form (see form footer).
    require_once("../include/check_post_key.php");

    // P1: whitelist of vars this form is allowed to write. Anything else
    // arriving in $_POST['cfg_*'] is rejected. (Previously a blacklist, which
    // failed open for any new config var added without updating the list.)
    // To expose a new var: add a form input AND add the var here.
    $writable = [
        // System / display
        'OJ_NAME','OJ_HOME','OJ_LANG','OJ_EXPIRY_DAYS','OJ_FRIENDLY_LEVEL',
        'OJ_TEMPLATE','OJ_CSS','OJ_LANGMASK','OJ_INDEX_NEWS_TITLE','OJ_BEIAN',
        // Email (SMTP_PASS is also gated by OJ_SaaS_ENABLE further below)
        'SMTP_SERVER','SMTP_PORT','SMTP_USER','SMTP_PASS',
        // Limits & UI behaviour
        'OJ_LOGIN_FAIL_LIMIT','OJ_REG_SPEED','OJ_POISON_BOT_COUNT',
        'OJ_KEEP_TIME','OJ_CONTEST_LIMIT_KEYWORD','OJ_NOIP_KEYWORD',
        'OJ_MARK','OJ_MARKDOWN','OJ_RANK_LOCK_DELAY','OJ_RANK_LOCK_PERCENT',
        'OJ_SUBMIT_COOLDOWN_TIME',
        // All form checkboxes (mirror of $bool_vars below)
    ...
    foreach ($bool_vars as $var) {
        // Only flip-to-false for vars that are in the whitelist (defense in depth).
        if (!in_array($var, $writable)) continue;
        $key = 'cfg_' . $var;
        if (!isset($_POST[$key])) {
            $count = 0;
    ...
    foreach ($_POST as $key => $raw_val) {
        ...
        $var = substr($key, 4);
        if (!in_array($var, $writable)) {
            $errors[] = "变量 \${$var} 不在允许列表中，无法通过此页面修改。";
            continue;
        }
